# -*- coding: utf-8 -*-
"""
CIS Project — Multimodal SER on MELD  (Version 3b)
====================================================
v3b = v2 speed + two targeted architecture upgrades:

  1. CrossAttentionFusion replaces sigmoid gate  (v2 → v3b)
     — v2: concat(audio,text) → sigmoid gate element-wise → MLP → 7 classes
     — v3b: audio attends to text, text attends to audio (bidirectional MHA)
            residual + LayerNorm on each, concat(256+256=512) → MLP → 7 classes
     — Allows each modality to dynamically query the other rather than a
       static per-element gate.

  2. FocalLoss(γ=2) replaces CrossEntropyLoss + label_smoothing=0.1  (v2 → v3b)
     — FL(p_t) = -(1−p_t)^γ · log(p_t), with class weights retained.
     — γ=2 exponentially down-weights easy majority examples (neutral ~47%)
       forcing the model to concentrate on hard minority classes.
     — Label smoothing removed; focal loss handles minority focus natively.

wav2vec2 remains fully FROZEN and audio features are pre-extracted to cache
(identical to v2). Reuses D:\\SEAD\\meld_cache\\meld_v2_*.pkl directly.
Training speed: same as v2 (~8 min/epoch on 4GB GPU).
...
# ================================================================
# PATHS  — reuses v2 embedding caches exactly
# ================================================================
...
# Reuse v2 embedding caches — no re-extraction needed
...
MODEL_SAVE_DIR = r"D:\SEAD\meld_cache\saved_models_meld_v3b"
...
# ================================================================
# Hyperparameters  (identical to v2 except FOCAL_GAMMA replaces LABEL_SMOOTH)
# ================================================================
TARGET_SR    = 16000
EPOCHS       = 15
BATCH_SIZE   = 32
...
WARMUP_FRAC  = 0.10
WEIGHT_DECAY = 1e-4
FOCAL_GAMMA  = 2.0         # replaces LABEL_SMOOTH=0.10
...
# ================================================================
# Audio utilities  (used only when cache is missing)
# ================================================================
...
class Wav2VecExtractor:
    """Frozen wav2vec2-base for one-time pre-extraction."""
...
# ================================================================
# Feature pre-extraction  (only runs if caches missing)
# ================================================================
...
class CrossAttentionFusion(nn.Module):
    ...
    audio-to-text attention  : audio queries, text keys/values
    text-to-audio attention  : text queries, audio keys/values
    Residual + LayerNorm → concat(256+256=512) → MLP → num_classes.
    """
    ...
    def forward(self, audio_vec, text_vec):
        ...
        a_ctx, _ = self.attn_a2t(a, t, t)            # audio queries text
        t_ctx, _ = self.attn_t2a(t, a, a)            # text queries audio

        a_out = self.norm_a(a + a_ctx).squeeze(1)    # (B, 256) residual
        t_out = self.norm_t(t + t_ctx).squeeze(1)    # (B, 256) residual

        return self.classifier(torch.cat([a_out, t_out], dim=1))  # (B, 512→cls)
...
class FocalLoss(nn.Module):
    """
    FL(p_t) = -weight_t · (1 − p_t)^γ · log(p_t)
    γ=2 heavily down-weights easy majority examples.
    Class weights retained for imbalance correction.
    """
...
# ================================================================
# Optimizer  (identical 2-group structure to v2)
# ================================================================
...
def print_results_table(dev_wf1, test_acc, test_wf1):
    print("\n" + "=" * 70)
    print("  MELD v3b RESULTS — wav2vec2 frozen + CrossAttn + FocalLoss")
    print("=" * 70)
...
def run_smoke_test():
    """
    Load 5 pre-extracted embeddings from the v2 test cache, build
    EmotionModel(bert), run one forward pass, confirm loss is not NaN.
    """
    print("=" * 55)
    ...
    for i, (t, l) in enumerate(zip(texts, labels)):
        print(f"  [{i}] label={label_encoder.classes_[l]}  text='{t}'  "
              f"embed_shape={np.array(embeds[i]).shape}")
    ...
    if is_nan:
        print("\n[FAIL] Loss is NaN — check model or loss function.\n")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smoke", action="store_true",
                        help="Run 5-sample forward-pass smoke test and exit.")
    ...
    # Step 2 — Load v2 embedding caches (or re-extract if missing)
